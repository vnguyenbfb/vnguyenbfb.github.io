...
The movie DVDs rental database can be downloaded [here](https://www.postgresqltutorial.com/wp-content/uploads/2019/05/dvdrental.zip). The database file is in zipformat ( dvdrental.zip) so you need to extract it to  dvdrental.tar before loading the sample database into the PostgreSQL database server. 

Printable ER diagram can be found [here](https://www.postgresqltutorial.com/wp-content/uploads/2018/03/printable-postgresql-sample-database-diagram.pdf). 
...
### 4. Analysis: Source Code, Visualization and Findings
#### 4.1. Compare numbers of rental orders by store
```
...
#### 4.2. Numbers of movies in each rental duration category
```
WITH table1 AS
...
#### 4.4.	Difference across the monthly payments during 2007 among the top 10 paying customers
```
WITH table1 AS
(   SELECT customer_id, SUM(amount) total_amount
    FROM payment
...
table3 AS
(   SELECT pay_mon, fullname, LEAD(fullname) OVER () AS next_fullname, pay_amount,
           LEAD(pay_amount) OVER () AS next_month_amount
    FROM table2)

SELECT pay_mon, fullname, pay_amount, next_month_amount,
       (next_month_amount - pay_amount) AS diff_fr_last_month
FROM table3
...
#### 4.5.	Count the times each film category has been rented and their average rental duration
```
